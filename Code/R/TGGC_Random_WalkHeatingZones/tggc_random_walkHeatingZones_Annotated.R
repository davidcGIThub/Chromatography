...
#for(i in 1:j)
#{   
# x[i,] <- sample(n.mol, size=n.mol, replace=F)/10 
#}
#write.table(x, "xdata.txt", sep="\t",row.names =FALSE, col.names=FALSE)
x <- matrix(scan("xdata.txt"),nrow=2,byrow=TRUE)
...
for(n in 0:run.time)
{
	...
	x <- x + ( u.m*delta.t + rnorm(j*n.mol, mean = 0, sd = sqrt(2*D.t) ) ) * as.numeric( runif(j*n.mol) < 1/(1+k.all ) ) 
  	...
  	if(n%%500==0)
 	{
      		...
      		if(max(x[2,])>col.length)
 		{
           		...
           		if(length(detected.2)+length(detected.1)-2 >= 2*n.mol)
 			{
 				break()
 			}
 		}
 	...
 	if(n%%50000==0)
 	{
     		...
     		par(mfrow=c(2,1), mar=c(2,1,1,6), las=1)# 2 graphs, margin bottom - left- top-right, tick marks perpindicular to axis
     		plot(0, 0, type='n', xlim=c(0,col.length), ylim=c(0, 1), yaxt='n',ylab='')# type - no points or lines, xlim,ylim,yaxt-suppress y axis, ylabel
     		mtext(text=paste(' Time:\n ', n*delta.t,' s'), side=4, cex=2)#write Time:n's, right margin,  #cex size of words
     		for(i in 1:j)
 		{
       			points(x[i,], (1:n.mol)/n.mol, col=cols[i], pch=i, cex=0.5) #(xpoints, ypoints, col = color,pch = x shapes, cex size points)
       			text(mean(x[i,]), 0.2+i*0.2, labels=round(4*sd(x[i,]),2 ), col=cols[i],pos=2,cex=2)#(xcoord,ycoord,labels-text written(rounds#'s(stanarddev())),color,position relative to coordinates,cex size)
     		}
     		lines(xx <-seq(1, col.length, 0.1),  (Temp.vec[xx*1e4]+tp.rate*delta.t*n)/200,col='red') #plot Tempvec at specified points
     		plot(density(x[1,]), type='n', xlim=c(0,col.length),ylim=c(0,1), xlab='',ylab='',main='') #(density plot,type-no plotting,xlimit,yaxt - suppress yaxis, xlabel,ylabel,main- title)
     		for(i in 1:j)
 		{
 			lines(density(x[i,]), col=cols[i] ) #plots lines on already existing graph
 		}
 		top.pks <- pk.head <- pk.tail <- numeric(j) #array of zeros
 		peakx <- numeric(1)
 		for(i in 1:j)
 		{
 			temp <- density(x[i,])
 			data <- temp$y #Gets the y data points for density curve
 			peakx <- which.max(data)  #finds the indice for the max in data[]
     ...
     for(i in 2:j)
 		{
       	res[i-1] <- abs( (top.pks[i]-top.pks[i-1]) / ( ( 1.7*(pk.head[i-1]-pk.tail[i-1]) + 1.7*(pk.head[i]-pk.tail[i]) )/2) )
     }
