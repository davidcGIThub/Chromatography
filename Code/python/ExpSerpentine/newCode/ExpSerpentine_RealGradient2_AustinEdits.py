...
#
#################
runTime = int(timeData[len(timeData)-1]/delta_t)  #Number of potential steps to get compounds out of column.
...
#for n in range(0,2):
    ...
    TempPlus = GData.getTemperatures(runTime*delta_t,TempGranularity)
    Tmax = np.amax(TempPlus)
    Tmin = np.amin(TempPlus)
    Tdelta = Tmax-Tmin
    # AUSTIN: All this computation of Tdelta is only for the scattered data point plot. NOT NECESSARY for actual separation computation
    
    temp = abs(x*TempGranularity+1)
    temp[temp > colLength*TempGranularity] = colLength*TempGranularity
    # AUSTIN: This converts the position of each molecule to an index value in the temperature array. The second line caps all values
    # so they can't extend beyond the end of the column. NOT NECESSARY FOR INTERP METHOD
    
    T_all = TempPlus[temp.astype(int)].flatten()
    # AUSITN: Compute temperature for each molecule. NOT NECESSARY FOR INTERP METHOD <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< GHANGE
    
    C_all = C    
    tmp_C_all = np.repeat([C_all],len(T_all)/len(C_all),axis=0).flatten('F')
    # AUSTIN: Creates a C array for each molecule
    
    tmp_h = np.repeat([h],len(T_all)/len(h),axis = 0).flatten('F')
    # AUSTIN: Computes an h value for each molecule
    
    k_all = np.exp(tmp_h/(R*T_all)+tmp_C_all)
    # AUSTIN: Computes a retention factor for each molecule
    ...
    # AUSTIN: Compute the temperature gradient at the given time step
    delta_t_internal = n*delta_t - int(n*delta_t)
    delta_T = 
    T_time = np.interp(

    
    TSumPow = np.cumsum(Temp2PowVec[(TempPlus*powerAce).astype(int)-1]) / TempGranularity
    # AUSTIN: This computes the integral of T^1.646 from 0 to x for all x positions <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< GHANGE
    ...
    totInt = TSumPow[int(colLength*TempGranularity)-1]  #This is the integral from 0 to L of T^1.646. <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< GHANGE
    ...
    # AUSTIN: This term is never used in the computations
    ...
    velocity_x = ( (p_i**2-p_o**2)*TempPlus[temp.astype(int)] * (diameter/2)**2 / ((16*HeFactor*totInt )*np.sqrt(abs(p_i**2-(p_i**2-p_o**2)*TSumPow[temp.astype(int)]/totInt ))) )
       # Velocity updated by HDT on 31 March 2015 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< GHANGE
       
    p_x = np.sqrt(abs((p_i**2-(p_i**2-p_o**2)*TSumPow[temp.astype(int)]/totInt)) ).flatten()
    # AUSTIN: copmute pressure then use in  velocity computation <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< GHANGE
    ...
    # AUSTIN: This computes the diffusion term for each molecule. Note that the randomization for the diffusion term happens in the next line. <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< GHANGE
    
    x = np.reshape((x.flatten() + ( velocity_x.flatten()*delta_t/(1 + k_all) + np.random.normal(0,sigmaDelta,j*nMol))),np.shape(x))
    # AUSTIN: This is the same equation as before. no changes should be needed here.
    ...
    # AUSTIN: The rest is just plot stuff
    if n%500==0:
